...
class TestOnglets:
    ...
    souhaite pas, dans un premier temps, que les enseignants puissent
    relancer un ré-encodage. Si cet onglet apparaissait ici, ce serait une
    régression de périmètre, pas une amélioration."""

    INTERDITS = {"myvids"}

    def test_l_onglet_mes_videos_reste_absent(self, app):
        presents = self.INTERDITS & set(app.tabs)
        assert not presents, (
            f"onglet(s) hors périmètre de la v2 : {presents}")


    ATTENDUS = {"upload", "config", "log"}
